...
for file in files:
    ...
    for key in models.keys():
        ...
        results[key] = {"train_observed_ddG": train_observed_ddG, "train_predict_ddG": train_predict_ddG, "train_combs": train_combs,
                        ...
                        "subs_observed_ddG": subs_observed_ddG, "subs_predict_ddG": subs_predict_ddG, "subs_test_combs": subs_test_combs,
                        "catsubs_observed_ddG": catsubs_observed_ddG, "catsubs_predict_ddG": catsubs_predict_ddG, "catsubs_test_combs": catsubs_test_combs}

    # Save Models
    with open(f"{file}_models.pickle", "wb") as f:
        pickle.dump(models, f)

    # Save Results
    with open(f"{file}_results.pickle", "wb") as f:
        pickle.dump(results, f)

    # Save to excel
    writer = pd.ExcelWriter(f"{file}_results.xlsx")
    excel = {}
